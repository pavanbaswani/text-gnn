...
def construct_graph(adjacency):
    ...
    g.ndata['d'] = torch.from_numpy(adjd.astype(np.float32)).to(device)
    g.edata['w'] = weight.to(device)
...
class GATLayer(nn.Module):
    ...
    def forward(self, feat):
        # equation (1)
        g = self.graph.local_var().to('cuda:0')
        g.ndata['h'] = feat.mm(getattr(self, 'W'))
...
g = construct_graph(adj).to(device)
...
with open('/home2/prateekj/GNN-for-text-classification/data/corpus/' + args.dataset + '_vocab.txt', 'r') as f:
    words = f.readlines()
...
word_embeddings_str = '\n'.join(word_vectors)
with open('/home2/prateekj/GNN-for-text-classification/data/' + args.dataset + '_word_vectors.txt', 'w') as f:
